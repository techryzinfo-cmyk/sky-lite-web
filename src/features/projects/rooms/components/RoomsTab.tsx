...
export const RoomsTab: React.FC<RoomsTabProps> = ({ projectId, project }) => {
  ...
  const canView = hasProjectPermission(user, project, 'rooms:view');
  const canCreate = !isLocked && hasProjectPermission(user, project, 'rooms:create');
  const canUpdate = !isLocked && hasProjectPermission(user, project, 'rooms:update');
  const canDelete = !isLocked && hasProjectPermission(user, project, 'rooms:delete');
  ...
  const handleSave = async (e: React.FormEvent) => {
    e.preventDefault();
    if (editing ? !canUpdate : !canCreate) { toast.error('You do not have permission to do this.'); return; }
  ...
  const handleDelete = async (room: any) => {
    if (!canDelete) { toast.error('You do not have permission to delete rooms.'); return; }
  ...
  if (!canView) {
    return (
      <div className="flex flex-col items-center justify-center py-24 text-center">
        ...
        <p className="text-sm font-bold text-slate-500">You don't have permission to view Rooms.</p>
      </div>
    );
  }
