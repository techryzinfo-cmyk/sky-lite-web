      ...
      <div className="mx-auto grid w-full max-w-5xl grid-cols-1 gap-6 lg:gap-8 lg:grid-cols-[1.2fr_1fr] my-auto">
        ...
        <section className="order-1 lg:order-2 self-center rounded-3xl border border-slate-200/80 bg-white p-6 lg:p-8 shadow-xl shadow-slate-200/60 flex flex-col justify-between">
          {showOtp ? <div>
            ...
            <form onSubmit={handleVerify} className="mt-6 space-y-4"><div><label className="text-sm font-semibold text-slate-700">Verification code</label><div className="relative mt-1.5"><KeyRound className="absolute left-3.5 top-1/2 size-4.5 -translate-y-1/2 text-slate-400" /><input type="text" maxLength={6} required value={otp} onChange={(event) => setOtp(event.target.value.replace(/\D/g, ''))} className={`${inputClass} pr-10 text-center font-mono text-base tracking-[0.45em]`} placeholder="000000" /></div></div><button type="submit" disabled={isVerifying || otp.length !== 6} className="flex w-full items-center justify-center gap-2 rounded-xl bg-blue-600 px-4 py-3.5 text-sm font-semibold text-white transition hover:bg-blue-500 disabled:cursor-not-allowed disabled:opacity-60">{isVerifying && <Loader2 className="size-4 animate-spin" />}{isVerifying ? 'Verifying...' : 'Verify workspace'}</button></form>
            <p className="mt-4 text-center text-sm text-slate-500">Didn&apos;t receive a code? <button type="button" onClick={handleResend} disabled={resendLoading} className="font-bold text-blue-600 hover:text-blue-700 disabled:opacity-50">{resendLoading ? 'Resending...' : 'Resend code'}</button></p>
          </div> : <>
            <div className="mb-5"><p className="text-xs font-semibold text-blue-600">CREATE YOUR WORKSPACE</p><h2 className="mt-1.5 text-2xl font-extrabold tracking-tight text-slate-900">Get started with SkyStruct Lite</h2><p className="mt-1.5 text-sm leading-normal text-slate-500">Set up your account now. Invite teammates and create your first project straight after.</p></div>
            <form onSubmit={handleSubmit} className="space-y-4">
              <div><label className="text-sm font-semibold text-slate-700">Full name</label><div className="relative mt-1.5"><User className="absolute left-3.5 top-1/2 size-4.5 -translate-y-1/2 text-slate-400" /><input type="text" value={name} onChange={(event) => setName(event.target.value)} className={inputClass} placeholder="John Doe" required /></div></div>
              <div><label className="text-sm font-semibold text-slate-700">Work email</label><div className="relative mt-1.5"><Mail className="absolute left-3.5 top-1/2 size-4.5 -translate-y-1/2 text-slate-400" /><input type="email" value={email} onChange={(event) => setEmail(event.target.value)} className={inputClass} placeholder="name@company.com" required /></div></div>
              <div>
                <label className="text-sm font-semibold text-slate-700">Mobile number</label>
                <div className="mt-1.5 w-full rounded-xl border border-slate-200 bg-slate-50 py-3 px-3 text-sm text-slate-900 outline-none transition focus-within:border-blue-500 focus-within:ring-2 focus-within:ring-blue-500/20">
                  <PhoneInput
                    international
                    defaultCountry="QA"
                    value={phoneNumber}
              ...
              <div className="grid gap-3 sm:grid-cols-2">
                <div><label className="text-sm font-semibold text-slate-700">Password</label><div className="relative mt-1.5"><Lock className="absolute left-3.5 top-1/2 size-4.5 -translate-y-1/2 text-slate-400" /><input type={showPassword ? 'text' : 'password'} value={password} onChange={(event) => setPassword(event.target.value)} className={inputClass} placeholder="Password" required /><button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-3.5 top-1/2 -translate-y-1/2 text-slate-400 hover:text-slate-900">{showPassword ? <EyeOff className="size-4.5" /> : <Eye className="size-4.5" />}</button></div></div>
                <div><label className="text-sm font-semibold text-slate-700">Confirm password</label><div className="relative mt-1.5"><Lock className="absolute left-3.5 top-1/2 size-4.5 -translate-y-1/2 text-slate-400" /><input type={showPassword ? 'text' : 'password'} value={confirmPassword} onChange={(event) => setConfirmPassword(event.target.value)} className={inputClass} placeholder="Confirm password" required /></div></div>
              </div>
              <button type="submit" disabled={isLoading} className="mt-2 flex w-full items-center justify-center gap-2 rounded-xl bg-blue-600 px-4 py-3.5 text-sm font-semibold text-white transition hover:bg-blue-500 disabled:cursor-not-allowed disabled:opacity-60">{isLoading && <Loader2 className="size-4 animate-spin" />}{isLoading ? 'Creating workspace...' : 'Create workspace'}</button>
            </form>
            <div className="mt-4 border-t border-slate-100 pt-3 text-center text-sm text-slate-500">Already have a workspace? <Link href="/login" className="font-semibold text-blue-600 hover:text-blue-700">Sign in</Link></div>
          </>}
          <div className="mt-4 flex items-center justify-center gap-2 text-xs text-slate-400"><CheckCircle2 className="size-3.5" /> Secure account setup for your organisation</div>
        </section>
      </div>
    </div>
  );
