...
import { X, Loader2, Layers, Plus, Trash2, DollarSign, FileText, Info } from 'lucide-react';
              ...
              <div className="p-8 max-h-[90vh] overflow-y-auto custom-scrollbar">
                <div className="flex items-center justify-between mb-8">
                  <div className="flex items-center space-x-3">
                    ...
                    <div>
                      <h2 className="text-xl font-bold text-white">Design New Blueprint</h2>
                      <p className="text-xs text-slate-400 mt-0.5">Create a reusable project template.</p>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="space-y-4">
                      <div className="space-y-2">
                        <label className="text-sm font-medium text-slate-300 ml-1">Template Name</label>
                        <input
                          type="text"
                          required
                          value={formData.name}
                    ...
                    <div className="space-y-3">
                      {formData.boqItems.map((item, index) => (
                        <div key={index} className="grid grid-cols-12 gap-3 items-end bg-white/[0.02] p-3 rounded-xl border border-white/5">
                          <div className="col-span-3 space-y-1">
                            <label className="text-[9px] font-black text-slate-600 uppercase tracking-widest ml-1">Group</label>
                            <input
                              type="text"
                          ...
                          <div className="col-span-2 space-y-1">
                            <label className="text-[9px] font-black text-slate-600 uppercase tracking-widest ml-1">Est. Rate</label>
                            <input
                              type="number"
                              value={item.rate}
                      ...
                      {formData.boqItems.length === 0 && (
                        <div className="py-8 text-center border border-dashed border-white/5 rounded-2xl">
                          <p className="text-xs text-slate-600">No BOQ structure added yet.</p>
                        </div>
                      )}
                    </div>
                  </div>

                  <div className="pt-6 flex space-x-4 border-t border-white/5">
                    <button
                      type="button"
                      onClick={onClose}
