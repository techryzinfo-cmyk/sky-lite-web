...
export const OverviewDashboard = () => {
  ...
  const stats = [
    ...
    { 
      ...
      value: '14', // Aggregated from issues
      icon: AlertCircle, 
      color: 'text-red-500', 
      bg: 'bg-red-500/10',
  ...
  if (loading) {
    return (
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {[1, 2, 3, 4].map(i => (
          <div key={i} className="h-32 rounded-3xl bg-slate-900/50 animate-pulse border border-white/5" />
        ))}
      </div>
    );
  }
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        {stats.map((stat, i) => (
          <GlassCard key={stat.name} className="p-6 border-white/5 group hover:border-blue-500/30 transition-all" gradient>
            ...
            <p className="text-[10px] font-bold text-slate-600 uppercase tracking-widest mt-2">{stat.trend}</p>
          </GlassCard>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <GlassCard className="lg:col-span-2 p-8 border-white/5" gradient>
          <div className="flex items-center justify-between mb-8">
            ...
            <button className="text-[10px] font-black text-blue-400 hover:text-blue-300 uppercase tracking-[0.2em] transition-colors">
              View Analytics
            </button>
          </div>
        ...
        <GlassCard className="p-8 border-white/5" gradient>
          ...
          <div className="space-y-6 relative">
            ...
            {projects.slice(0, 5).map((project, i) => (
              <div key={i} className="flex items-start space-x-4 relative">
                <div className="w-3 h-3 rounded-full bg-blue-500 border-4 border-[#0F172A] z-10 mt-1" />
                <div>
                  <p className="text-sm text-white font-medium line-clamp-1">
                    {project.auditTrail?.[0]?.details || `Project ${project.name} was updated`}
                  </p>
                  <div className="flex items-center space-x-2 mt-1">
                    <span className="text-[10px] font-bold text-blue-500 uppercase">{project.name}</span>
          ...
          <button className="w-full mt-8 py-3 rounded-2xl bg-white/5 border border-white/5 text-xs font-bold text-slate-400 hover:bg-white/10 hover:text-white transition-all">
            View All Activity
          </button>
        </GlassCard>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        <GlassCard className="p-6 border-white/5 group hover:border-emerald-500/30 transition-all cursor-pointer" gradient>
          ...
          <h4 className="text-lg font-bold text-white mb-1">BOQ Compliance</h4>
          <p className="text-sm text-slate-500">Track variance between estimated and actual quantities.</p>
        </GlassCard>

        <GlassCard className="p-6 border-white/5 group hover:border-amber-500/30 transition-all cursor-pointer" gradient>
          <div className="flex items-center justify-between mb-6">
          ...
          <h4 className="text-lg font-bold text-white mb-1">Handover Readiness</h4>
          <p className="text-sm text-slate-500">View snagging resolution status across all completion stages.</p>
        </GlassCard>

        <GlassCard className="p-6 border-white/5 group hover:border-purple-500/30 transition-all cursor-pointer" gradient>
          <div className="flex items-center justify-between mb-6">
            <div className="p-3 rounded-2xl bg-purple-500/10 border border-purple-500/20">
              <PieChart className="w-6 h-6 text-purple-400" />
            </div>
            <ChevronRight className="w-5 h-5 text-slate-700 group-hover:text-purple-500 group-hover:translate-x-1 transition-all" />
          </div>
          <h4 className="text-lg font-bold text-white mb-1">Financial Intelligence</h4>
